...
void apparier(char c1, char& r1, char& r2, char& r3, int& score)
{
  ...
  verifier(c1, r2, score);
  verifier(c1, r3, score);
}
                       ...
                       char r1, char r2, char r3, char r4)
{
  ...
  cout << chk[0] << chk[1] << chk[2] << chk[3] << endl;
  cout << gus[0] << gus[1] << gus[2] << gus[3] << endl;

  for (int j=0;j<4;j++) {
    if (gus[0]!='x') apparier(gus[0], ref[1], ref[2], ref[3], rcwp);
    else if (gus[1]!='x') apparier(gus[1], ref[0], ref[2], ref[3], rcwp);
    else if (gus[2]!='x') apparier(gus[2], ref[0], ref[1], ref[3], rcwp);
    else if (gus[3]!='x') apparier(gus[3], ref[0], ref[1], ref[2], rcwp);
  }
